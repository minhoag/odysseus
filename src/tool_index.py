...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "serve_model": "Start serving a model. cmd MUST start with one of: vllm, python3, llama-server, ollama, sglang, lmdeploy, node, npx. NEVER prefix with `cd`, `source`, or chain with `&&`/`||` — rejected by validator. Venv (env_prefix) and CUDA env auto-added from the host's settings.\n\nPick the binary by model format:\n  • GGUF (repo ends `-GGUF`, has `gguf`): use llama-server, NOT `python3 -m llama_cpp.server`. Example: `llama-server --hf-repo unsloth/Qwen3.5-9B-GGUF --host 0.0.0.0 --port 8000 --jinja -np 1 --n-gpu-layers 99`. CRITICAL flags for newer GGUFs (Gemma 4, Qwen3, etc.):  `--jinja` uses the chat template embedded in the GGUF (the shorthand `--chat-template gemma`/etc. is for OLDER versions — Gemma 4 needs --jinja or it never emits EOS and loops repeating itself).  `-np 1` single slot so the picker probe and chat don't share KV state.\n  • HF transformers / AWQ on GPU: `vllm serve <repo_or_path> --host 0.0.0.0 --port 8000 --tensor-parallel-size <N_gpus>`\n  • Ollama-pulled tag (e.g. qwen2.5:0.5b): `OLLAMA_HOST=0.0.0.0:11434 ollama serve` (model is selected at chat time, not in this cmd)\n  • Diffusion / inpainting / image: `python3 scripts/diffusion_server.py --model <repo> --port 8100`\n\nArgs (JSON): {\"repo_id\": \"<HF repo or local path>\", \"cmd\": \"<full cmd above>\", \"host\": \"kierkegaard\" or \"pewds@192.168.1.12\"}. If cmd is omitted entirely the backend auto-fills a sensible default based on repo_id. After launch, call list_served_models for readiness — if it returns a diagnosis with an adjusted command, retry with that command. If serve fails with 'Invalid characters in cmd', simplify to the bare binary + args.",
    ...
    "tail_serve_output": "Read the actual tmux stderr/traceback of a cookbook serve/download task. Use to debug WHY a task is `crashed`/`error` (compute_89 nvcc mismatch, OOM, missing kernels, wrong attention backend, etc.) so you can call serve_model with adjusted flags. Pass session_id from list_served_models; tail defaults to 300, bump if the error references 'see root cause above'. DO NOT poll this in a loop while a model is loading — call it ONCE per turn at most. If output shows in-progress loading (e.g. weight shards copying `N/M`, vLLM `[gpu_model_runner] Starting to load`, `Loading checkpoint shards`), end the turn with [DONE] and a one-line ETA — the user will return when ready, or a self-wake fires if you provide an explicit time like 'check in ~3 min'. Repeated tails on the same loading task pin the upstream SSH and burn tokens.",
